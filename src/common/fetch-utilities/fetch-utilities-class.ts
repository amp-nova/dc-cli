...
export class FetchUtilities {
   ...
   */
    ...
    const response = await fetch(authUrlFinal, { method: 'POST', headers: { "Content-Type": "application/x-www-form-urlencoded" } });
  ...
  async fetchResource(endpointUri: string): Promise<any> {
    ...
    if (this.accessToken) {
      ...
      const response = await fetch(`${this.apiUrl}${endpointUri}`, { method: 'GET', headers: { "Authorization" : "Bearer " + this.accessToken } });
      if (response.ok) {
   ...
   */
  async fetchPaginatedResourcesList(
      endpointUri: string, 
   ...
   * @param resourceName Name of the embedded field for pagination
   */
  async fetchPaginatedResourcesListWithDetails(
    endpointUri: string, 
    resourceName: string): Promise<any[]> {

    let resourcesList = await this.fetchPaginatedResourcesList(endpointUri, resourceName);

    // Retrieve all resources in parallel
    let resourcesListDetails = await Promise.all(
      resourcesList.map(
        (item: any) => 
  ...
  async deleteResource(
    endpointUri: string, 
    resourceId: string): Promise<string> {
