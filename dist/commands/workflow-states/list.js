...
exports.desc = 'List Workflow States';
exports.builder = {
    ...sorting_options_1.SortingOptions,
    ...data_presenter_1.RenderingOptions
};
exports.itemMapFn = ({ id, label, color }) => {
    return { ID: id, Label: label, Color: color };
};
exports.handler = async (argv) => {
    const client = dynamic_content_client_factory_1.default(argv);
    const hub = await client.hubs.get(argv.hubId);
    const workflowStatesList = await paginator_1.default(hub.related.workflowStates.list, sorting_options_1.extractSortable(argv));
    new data_presenter_1.default(workflowStatesList.map(value => value.toJSON())).render({
        json: argv.json,
